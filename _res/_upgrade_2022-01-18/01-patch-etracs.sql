...
update 
  ...
  ( 
    select 
      b.objid, b.yearstarted, 
      min(ba.yearstarted) as app_yearstarted 
    from business b, business_application ba 
    where ba.business_objid = b.objid 
    group by b.objid, b.yearstarted 
    having b.yearstarted <> min(ba.yearstarted) 
  )bb 
set
  aa.yearstarted = bb.app_yearstarted
where 
  aa.objid = bb.objid
; 

update 
  business_application aa, 
  business bb 
set 
  aa.yearstarted = bb.yearstarted 
where 
  aa.business_objid = bb.objid and 
  aa.yearstarted <> bb.yearstarted 
; 
