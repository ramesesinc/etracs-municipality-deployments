...
drop table if exists ztmp_duplicate_usergroupmember 
; 
create table ztmp_duplicate_usergroupmember 
select ugm.* 
from ( 
  select tmp1.*, 
    (
      ...
      where user_objid = tmp1.user_objid
        ...
        and ifnull(org_objid,'') = ifnull(tmp1.org_objid,'') 
      limit 1
    ) as ugm_objid 
  from ( 
...
where ugm.user_objid = tmp2.user_objid 
  ...
  and ifnull(ugm.org_objid,'') = ifnull(tmp2.org_objid,'') 
