...
from ( 
  select  
    'SRE' as objid, 'SRE' as title, 0 as version, 
...
from ( 
  select 
    objid, 'SRE' as maingroupid, code, title, parentid as groupid, 
    'root' as type, null as leftindex, null as rightindex, null as `level` 
  from sreaccount where parentid is null
  union all 
  select 
    a.objid, 'SRE' as maingroupid, a.code, a.title, null as groupid, 
...
update 
  ...
  ( 
    select 
      a.objid, 'SRE' as maingroupid, a.parentid as groupid 
...
alter table collectiongroup_account add (
...
select distinct 
  bai.lobid as objid, 'INACTIVE' as state, bai.name, 
  (select objid from lobclassification where objid like 'OTHER%' order by objid limit 1) as classification_objid 
...
drop table if exists ztmp_invalid_business_permit
;
...
;
drop table ztmp_invalid_business_permit
;
...
create index ix_ukey on cashreceipt (_ukey); 
update cashreceipt set _ukey=MD5(objid);
...
; 
alter table cashreceipt 
  modify controlid varchar(50) character set latin1 not null 
; 
...
update depositvoucher set state = 'POSTED' where controldate < '2021-09-18' and state = 'OPEN'
;
...
set 
  aa.controlid = bb.controlid, 
  aa.receivedstartseries = bb.receivedstartseries, aa.receivedendseries = bb.receivedendseries, aa.qtyreceived = bb.qtyreceived, 
  aa.beginstartseries = bb.beginstartseries, aa.beginendseries = bb.beginendseries, aa.qtybegin = bb.qtybegin, 
  aa.issuedstartseries = bb.issuedstartseries, aa.issuedendseries = bb.issuedendseries, aa.qtyissued = bb.qtyissued, 
  aa.endingstartseries = bb.endingstartseries, aa.endingendseries = bb.endingendseries, aa.qtyending = bb.qtyending, 
  aa.qtycancelled = bb.qtycancelled, aa.remarks = bb.remarks 
where aa.objid = bb.objid 
;
...
update 
  ...
  ( 
    select objid, 
      (beginendseries - beginstartseries + 1) as qtybegin 
    from af_control_detail 
    where reftype = 'remittance' 
      and txntype = 'remittance' 
      and beginstartseries > 0 
      and qtybegin = 0 
  )bb 
set aa.qtybegin = bb.qtybegin 
where aa.objid = bb.objid 
; 
...
update 
  ...
  ( 
    select t0.objid, 
      d.endingstartseries as receivedstartseries, d.endingendseries as receivedendseries, d.qtyending as qtyreceived
    from ( 
      select d.objid,  
        convert(d.refdate, date) as refdate, 
        (
          select convert(refdate, date) 
          from af_control_detail
          where controlid = d.controlid 
            and refdate <= d.refdate 
            and reftype = 'ISSUE' 
      ...
      where reftype = 'REMITTANCE' 
        and txntype = reftype 
        and qtyissued > 0 
    )t0, af_control_detail d 
    where t0.refdate = t0.dtissued 
      and d.objid = t0.prevdetailid 
  )bb 
set
  aa.receivedstartseries = bb.receivedstartseries, 
  aa.receivedendseries = bb.receivedendseries, 
  aa.qtyreceived = bb.qtyreceived, 
