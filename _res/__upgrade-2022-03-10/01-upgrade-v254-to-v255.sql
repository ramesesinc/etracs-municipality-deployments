...
update 
  ...
  ( 
    select afc.objid, 
      (select convert(min(receiptdate), date) from cashreceipt where controlid = afc.objid) as receiptdate  
    from af_control afc 
...
drop index uix_reqno on afrequest
;
...
;
alter table afrequest add _ukey varchar(50) not null default '';
update afrequest set _ukey = objid;
create unique index uix_reqno on afrequest (reqno, _ukey);
...
drop index uix_issueno on aftxn
; 
insert into aftxn ( 
...
from ( 
  ...
  inner join z20181120_stockissue si on si.objid = t1.refid 
;
alter table aftxn add _ukey varchar(50) not null default '';
update aftxn set _ukey = objid;
create unique index uix_issueno on aftxn (controlno, _ukey);
...
update cashreceiptitem set item_code='-' where item_code is null ;
...
-- ;
-- alter table collectiongroup_account add `valuetype` varchar(50) NULL
-- ;
-- alter table collectiongroup_account add `defaultvalue` decimal(16,2) NULL 
-- ;
...
create table ztmp_20220330_invalid_business_payment 
select bp.* 
from business_payment bp 
  left join business_application ba on ba.objid = bp.applicationid 
where ba.objid is null 
;
delete from business_payment where objid in (
  select objid from ztmp_20220330_invalid_business_payment
)
;
...
create table ztmp_20220330_invalid_business_payment_item
select bpi.* 
from business_payment_item bpi 
  left join business_payment bp on bp.objid = bpi.parentid 
where bp.objid is null 
; 
delete from business_payment_item where objid in (
  select objid from ztmp_20220330_invalid_business_payment_item 
)
;
...
create table ztmp_20220330_invalid_business_receivable
select br.* 
...
delete from business_receivable where objid in (
  select objid from ztmp_20220330_invalid_business_receivable
)
;
...
-- create index `ix_subcollector_objid` on cashreceipt (`subcollector_objid`); 
-- create index `ix_txndate` on cashreceipt (`txndate`); 
...
create table ztmp_20220330_patch_cashreceipt 
select c.* 
from cashreceipt c
  left join af_control afc on afc.objid = c.controlid 
where c.controlid is not null 
  and afc.objid is null 
;
insert into af_control (
  objid, afid, txnmode, active, assignee_objid, assignee_name, owner_objid, owner_name, 
  startseries, endseries, currentseries, stubno, prefix, suffix, dtfiled, state, unit, batchno 
)  
select 
  z.controlid as objid, z.formno as afid, 'ONLINE' as txnmode, 0 as active, 
  z.collector_objid as assignee_objid, z.collector_name as assignee_name, 
  z.collector_objid as owner_objid, z.collector_name as owner_name, 
  min(z.series) as startseries, max(z.series) as endseries, max(z.series)+1 as currentseries, 
  z.stub as stubno, '' as prefix, '' as suffix, convert(min(receiptdate), date) as dtfiled, 
  'ISSUED' as state, 'STUB' as unit, 1 as batchno 
from ztmp_20220330_patch_cashreceipt z 
group by 
  z.controlid, z.formno, z.collector_objid, z.collector_name, z.stub  
;
drop table ztmp_20220330_patch_cashreceipt
;
...
alter table collectiontype_account drop foreign key FK_collectiontype_account; 
...
alter table itemaccount_tag add objid varchar(50) not null default ''; 
update itemaccount_tag set objid = CONCAT('ACCTTAG-', UUID()); 

create index ix_acctid on itemaccount_tag (acctid) ; 
create index ix_tag on itemaccount_tag (tag); 
alter table itemaccount_tag drop primary key; 
...
-- alter table sys_org add txncode varchar(10) null; 
...
alter table sys_rule_action_param add rangeoption int null ; 
